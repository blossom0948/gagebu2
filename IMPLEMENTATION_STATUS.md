...
- Worker 경로: `/health`, `/v1/parse-transaction`, `/v1/analyze-spending`, `/v1/classify-notification`
...
- `./gradlew :app:testDebugUnitTest :app:lintDebug :app:assembleDebug` — 성공
- `./gradlew :app:assembleRelease` (VERSION_CODE=10, VERSION_NAME=0.1.10) — 성공 (`lintVitalRelease` 포함)
- APK 메타데이터 확인: `com.moasseum.app`, `versionCode=8`, `versionName=0.1.8`; 서명 인증서가 로컬 릴리스 키와 일치
- GitHub Release `v0.1.10` 및 `app-release.apk` 업로드 완료: https://github.com/blossom0948/gagebu2/releases/tag/v0.1.10
...
- 서명된 Release APK — `app/build/outputs/apk/release/app-release.apk` (56,692,202 bytes; 오프라인 한국어 OCR 모델 포함)
